...
//		elemet emeljük a megfelelő hatványra, majd írjuk ki!
		...
		for (int i = 0; i < felhasznaloTombje.length; i++) {
			System.out.println(
					"Adjon meg egy egész számot " + (i + 1) + "/" + felhasznaloTombje.length);
		...
		System.out.println("Az utolsó szám négyzete: "
				+ (int) (Math.pow(felhasznaloTombje[felhasznaloTombje.length - 1], 2.0)));


		System.out.println("\n\nc. Írjuk ki az első 2 elem átlagát!");
		double elsoKettoAtlga = (double) (felhasznaloTombje[0] + felhasznaloTombje[1]) / 2.0;
		...
		for (int i = felhasznaloTombje.length - 1; i > felhasznaloTombje.length - 4; i--) {
		...
		System.out.println("Az első és a második elem különbsége " + kulonbsegEgyKEtto + ".\n");


		System.out.println("\n");
		System.out.println("f. Kérjünk be a felhasználótól egy indexet, és egy hatványkitevőt. "
				+ "A megadott indexen lévő elemet emeljük a megfelelő hatványra, majd írjuk ki!\n");

		System.out.print("Adja meg az indexet amit hatványozni "
				+ "szeretne(nagyobb vagy egyenlő 0, kisebb , mint " + felhasznaloTombje.length
				+ "): ");
		int bekertIndex = Integer.parseInt(scanner.nextLine());

		System.out.print("Adja meg melyik hatványra emelné (pozitív, egész szám): ");
		int bekertPow = Integer.parseInt(scanner.nextLine());

		int indexHatvanyozva = (int) Math.pow(felhasznaloTombje[bekertIndex], bekertPow);
		System.out.println("A bekért indexen lévő szám (" + felhasznaloTombje[bekertIndex]
				+ ") a megadott hatványkitevőre (" + bekertPow + ") emelve: " + indexHatvanyozva);
